...
class Panel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        ...
        self.labels['tip'].set_text("Calibrate z probe")
        ...
        pos.attach(self.widgets['zposition'], 0, 0, 1, 2)
        if self.z_offset is not None:
            self.widgets['zoffset'] = Gtk.Label(label="?")
            # pos.attach(Gtk.Label(_("Probe Offset") + ": "), 2, 0, 2, 1)
            pos.attach(Gtk.Label(_("Saved")), 1, 0, 1, 1)
            pos.attach(Gtk.Label(_("New")), 2, 0, 1, 1)
            pos.attach(Gtk.Label(f"{self.z_offset:.3f}"), 1, 1, 1, 1)
            pos.attach(self.widgets['zoffset'], 2, 1, 1, 1)
        self.buttons = {
        ...
        # else:
        self.buttons['start'].connect("clicked",  self.calibrate_probe)
        ...
        if self._screen.vertical_mode:
            ...
            grid.attach(self.buttons['zpos'], 0, 1, 1, 1)
            grid.attach(self.buttons['zneg'], 0, 2, 1, 1)
            grid.attach(self.buttons['start'], 2, 1, 1, 1)
            grid.attach(pos, 1, 2, 3, 1)
            grid.attach(self.buttons['complete'], 4, 1, 1, 1)
            grid.attach(self.buttons['cancel'], 4, 2, 1, 1)
            grid.attach(distances, 0, 3, 5, 2)
    ...
    def calibrate_probe(self, widge):
        self._move_to_position()
        self._screen._ws.klippy.gcode_script("PROBE_CALIBRATE")
    ...
    def on_next_click(self, widget=None):
        self._screen.show_panel("zcalibrate_mesh", "Calibrate Zmesh", remove_all=True)         
