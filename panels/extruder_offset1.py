...
class Panel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        ...
        self.pos = {}
        self.menu = ['move_menu']
        self.klipper_config = os.path.join(os.path.expanduser("~/"), "printer_data", "klipper.conf")
        self.pos['e1_xoffset'] = self._config.klippy_config.get("Variables", "e1_xoffset", fallback=-61.8)
        self.pos['e1_yoffset'] = self._config.klippy_config.get("Variables", "e1_yoffset", fallback=0.0)
        self.pos['e1_zoffset'] = self._config.klippy_config.get("Variables", "e1_zoffset", fallback=0.0)
        ...
        self.labels['tip'].set_text("Extruder offset measurement")
        next_btn = self._gtk.Button("complete", None, "color1", .66)
        next_btn.connect("clicked", self.on_complete_click)
        ...
        if self._screen.vertical_mode:
            ...
            if self._screen.lang_ltr:
                grid.attach(self.buttons['x+'], 2, 2, 1, 1)
        ...
        grid.attach(self.buttons['home'], 0, 1, 1, 1)
        grid.attach(self.buttons['motors_off'], 2, 1, 1, 1)
        ...
        for extruder in self._printer.get_tools():
            ...
            if i < limit:
                grid.attach(self.labels[extruder], 4, i+1, 1, 1)
                i += 1
        ...
        # offsetgrid = Gtk.Grid()
        # offsetgrid.set_row_homogeneous(False)
        self.labels['confirm'] = self._gtk.Button(None, "Confirm", "color1")
        self.labels['right'] = self._gtk.Button(None, "Confirm", "color1")
        self.labels['save'] = self._gtk.Button(None, "Save", "color1")
        self.labels['offset'] = Gtk.Label(label = "x: ? y: ? z: ?")

        self.labels['confirm'].connect("clicked", self.confirm_extrude_position)
        # self.labels['right'].connect("clicked", self.confirm_extrude_position, 1)
        self.labels['save'].set_sensitive(False)
        self.labels['save'].connect("clicked", self.save_offset)

        self.labels['step_text'] = Gtk.Label(label = "Step 1: let left extruder moves to a point on the hotbed.")
        # self.labels['step_text'].set_justify(Gtk.Justification.LEFT)
        # self.labels['step2_text'] = Gtk.Label(label = "Step 2: let right extruder moves to the same position.")
        offsetgrid.attach(self.labels['step_text'], 0, 0, 4, 1)
        offsetgrid.attach(self.labels['confirm'], 4, 0, 1, 1)
        # offsetgrid.attach(self.labels['step2_text'], 0, 1, 4, 1)
        # offsetgrid.attach(self.labels['right'], 4, 1, 1, 1)
        offsetgrid.attach(self.labels['offset'], 0, 1, 4, 1)            
        offsetgrid.attach(self.labels['save'], 4, 1, 1, 1)            
        
        self.labels['move_menu'] = self._gtk.HomogeneousGrid()
        self.labels['move_menu'].attach(grid, 0, 0, 1, 3)
        # self.labels['move_menu'].attach(bottomgrid, 0, 2, 1, 1)
        self.labels['move_menu'].attach(distgrid, 0, 3, 1, 1)
        self.labels['move_menu'].attach(offsetgrid, 0, 4, 1, 2)
    ...
    def process_update(self, action, data):
        ...
        text = ""
        for p in ('x', 'y', 'z'):
            if self.pos[p] is None:
                text  += f"{p} : ?         "
            else:
                text += f"{p}: {self.pos[p]:.2f}         "
        self.labels['tip'].set_markup(text)
    ...
    def home(self, widget):
        ...
        if "cartesian" in self._printer.get_config_section("printer")['kinematics']:
    ...
    def confirm_extrude_position(self, widget):
        if self._printer.extrudercount < 2:
            self._screen.show_popup_message("Only one extruder does not require calibration.")
            return
        self.current_extruder = self._printer.get_stat("toolhead", "extruder")

        # if self._printer.get_tool_number(self.current_extruder) != extruder_idx:
        #     self._screen.show_popup_message(f"The current extruder is not extruder{extruder_idx}.")
        #     return
                    
        # text = self.labels['pos_x'].get_text() + self.labels['pos_y'].get_text() + self.labels['pos_z'].get_text()
        # homed_axes = self._printer.get_stat("toolhead", "homed_axes")
        # if homed_axes != "xyz" or "?" in text: 
        if self.pos['lx'] is None:
            self._screen.show_popup_message("Unable to get x pos.")
            return
        if self.pos['ly'] == None:
            self._screen.show_popup_message("Unable to get y pos.")
            return
        if self.pos['ly'] == None:
            self._screen.show_popup_message("Unable to get z pos.")
            return
        else:
            if self._printer.get_tool_number(self.current_extruder) == 0:
                ...
                self.labels['step_text'].set_markup("Step 2: let right extruder moves to the same position.")
                self.labels['offset'].set_markup(f"left extruder pos: ({self.pos['lx']:.2f}, {self.pos['ly']:.2f}, {self.pos['lz']:.2f})")
            else:
                if self.pos['lx'] is None or self.pos['ly'] is None or self.pos['lz'] is None:
                    self._screen.show_popup_message(f"Please complete step1 first.")
                else:
                    self.pos['ox'] = self.pos['x'] - self.pos['lx']
                    self.pos['oy'] = self.pos['y'] - self.pos['ly']
                    self.pos['oz'] = self.pos['z']  - self.pos['lz']
                    self.labels['offset'].set_markup(f"Right extruder offset is ({self.pos['ox']:.2f}, {self.pos['oy']:.2f}, {self.pos['oz']:.2f})")
    ...
    def change_extruder(self, widget, extruder):
        ...
        if self._printer.get_tool_number(extruder) == 0:
            self.labels['step_text'] = Gtk.Label(label = "Step 1: let left extruder moves to a point on the hotbed.")
        else:                
            self.labels['step_text'] = Gtk.Label(label = "Step 2: let right extruder moves to the same position.")
        
    def save_offset(self, widget):
        if self.pos['ox'] is None or self.pos['oy'] is None or self.pos['oz'] is None:
            self._screen.show_popup_message("Need to recalculate the offset value.")
            # self.labels['offset'].set_markup("Need to recalculate the offset value")
        else:
            self.pos['e1_xoffset'] -= self.pos['ox']
            self.pos['e1_yoffset'] -= self.pos['oy']
            self.pos['e1_zoffset'] -= self.pos['oz']            
            self._config.klippy_config.set("Variables", "e1_xoffset", f"{self.pos['e1_xoffset']:.2f}")
            self._config.klippy_config.set("Variables", "e1_yoffset", f"{self.pos['e1_yoffset']:.2f}")
            self._config.klippy_config.set("Variables", "e1_zoffset", f"{self.pos['e1_zoffset']:.2f}")

            klipper_config = os.path.join(os.path.expanduser("~/"), "printer_data", "klipper.conf")
            try:
                with open(klipper_config, 'w') as file:
                    self._config.klippy_config.write(file)
            except Exception as e:
                logging.error(f"Error writing configuration file in {klipper_config}:\n{e}")
    ...
    def on_back_click(self, widget=None):
        self._screen.show_panel("zcalibrate_mesh", "Z calibration with bed mesh", remove_all=True)
    ...
    def on_complete_click(self, widget=None):
        ...
        self._config.klippy_config.set("Variables", "setup_init", "0")

        try:
            with open(self.klipper_config, 'w') as file:
                self._config.klippy_config.write(file)
        except Exception as e:
            logging.error(f"Error writing configuration file in {self.klipper_config}:\n{e}")
            self._screen.show_popup_message("Error writing configuration")                  